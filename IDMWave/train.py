...
if rank == 0:
    ...
    print("Using IDM-WAVE model with Wavelet Inputs and CBAM Attention.")
...
if len(training_image_paths) == 0:
    # 尝试备用路径（如果是用户本地环境）
    training_image_paths = glob.glob(os.path.join(args.data_dir, "train") + "/*")
...
# 加载 SD v1.5 的 U-Net 权重
pipe = StableDiffusionPipeline.from_pretrained("sd-legacy/stable-diffusion-v1-5").to(device)

# --- 修改点：实例化 NetWAVE ---
# 注意：这里会触发 NetWAVE 的重构逻辑，插入小波注入器和 CBAM 模块
base_model = Net(T, pipe.unet).to(device)
