...
class Step(RevModule):
    ...
    def body(self, x):
        with torch.cuda.amp.autocast(enabled=use_amp, cache_enabled=False):
            ...
            # --- (0) Noise prediction ---
            e = F.pixel_shuffle(
                unet(F.pixel_unshuffle(x, 2)), 2
            )

            # --- (1) Denoising: xt -> x0 ---
            x = (x - (1 - cur_alpha_bar).sqrt() * e) / cur_alpha_bar.sqrt()

            # =================================================
            # (2) Step-aware data consistency (Analytical)
            # =================================================
            lambda_t = net.compute_lambda_t(cur_alpha_bar)
            x = x - lambda_t * AT(A(x) - y)

            # --- (3) DDIM sampling ---
            x = prev_alpha_bar.sqrt() * x + (1 - prev_alpha_bar).sqrt() * e
            return x
...
class Net(nn.Module):
    def __init__(self, T, unet):
        ...
        # ===== Step-Aware Data Consistency (Analytical) =====
        self.lambda_max = nn.Parameter(torch.tensor(1.0))  # λ_max
        self.lambda_gamma = nn.Parameter(torch.tensor(1.0))  # γ
        self.unet = unet
    ...
    def forward(self, y_, A_, AT_, use_amp_=True):
        global y, A, AT, unet, ATy, alpha_bar, use_amp,net
        net = self
    ...
    def compute_lambda_t(self, alpha_bar_t):
        """
        Analytical step-aware data consistency weight:
        λ_t = λ_max * (1 - ᾱ_t)^γ
        """
        return self.lambda_max * (1.0 - alpha_bar_t).clamp(min=0).pow(self.lambda_gamma)
